...
describe('Payments API (/api/payments)', () => {
    ...
    before(async () => {
        try {
            ...
            const cli = await createUserAndLogin({
                email: `cli.pay.${Date.now()}@example.com`,
                password: 'password',
                role: 'cliente',
                full_name: 'Cliente Pay'
            });
            clienteId = cli.userId;
            clienteToken = cli.token;

            const art = await createUserAndLogin({
                email: `art.pay.${Date.now()}@example.com`,
                password: 'password',
                role: 'artigiano',
                full_name: 'Artigiano Pay',
                shop_name: 'Art Pay Shop'
            });
            ...
            const prodRes = await db.query(
                `INSERT INTO products (artisan_id, category_id, name, price, stock_quantity, is_active)
                 VALUES ($1, $2, 'Prod Pagamenti', 55.55, 10, TRUE)
    ...
    describe('POST /api/payments', () => {
        ...
        it('NON dovrebbe registrare pagamento con transaction_id duplicato', async () => {
            const duplicateTransId = `TRANS-DUP-${Date.now()}`;
            ...
            const firstPayment = {
                ...
                status: 'completed',
                transaction_id: duplicateTransId
            };
            await request(app)
                .post('/api/payments')
                .set('Authorization', `Bearer ${adminToken}`)
        ...
        requiredPaymentFields.forEach(field => {
            it(`NON dovrebbe registrare senza campo obbligatorio: ${field}`, async () => {
                const paymentDataBase = {
                    ...
                    status: 'pending',
                    transaction_id: `TRANS-MISS-${field}-${Date.now()}`
                };
    ...
    describe('GET /api/payments/order/:order_id', () => {
        ...
        it('Cliente NON Proprietario: NON dovrebbe ottenere dettagli (403)', async () => {
            const otherCli = await createUserAndLogin({
                email: `othercli.pay.${Date.now()}@example.com`,
                password: 'password',
                role: 'cliente',
                full_name: 'Other Cliente Pay'
            });
