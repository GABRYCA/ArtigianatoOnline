...
describe('Orders API (/api/orders)', () => {
    ...
    before(async () => {
        try {
            ...
            const cli1 = await createUserAndLogin({
                email: `cli1.ord.${Date.now()}@example.com`,
                password: 'password',
                role: 'cliente',
                full_name: 'Cliente 1 Ord'
            });
            cliente1Id = cli1.userId;
            cliente1Token = cli1.token;

            const cli2 = await createUserAndLogin({
                email: `cli2.ord.${Date.now()}@example.com`,
                password: 'password',
                role: 'cliente',
                full_name: 'Cliente 2 Ord'
            });
            cliente2Id = cli2.userId;
            cliente2Token = cli2.token;

            const art1 = await createUserAndLogin({
                email: `art1.ord.${Date.now()}@example.com`,
                password: 'password',
                role: 'artigiano',
                full_name: 'Artigiano 1 Ord',
                shop_name: 'Art1 Ord Shop'
            });
            artigiano1Id = art1.userId;
            artigiano1Token = art1.token;

            const art2 = await createUserAndLogin({
                email: `art2.ord.${Date.now()}@example.com`,
                password: 'password',
                role: 'artigiano',
                full_name: 'Artigiano 2 Ord',
                shop_name: 'Art2 Ord Shop'
            });
            ...
            const prodARes = await db.query(
                `INSERT INTO products (artisan_id, category_id, name, price, stock_quantity, is_active)
                 VALUES ($1, $2, 'Prod A Ord', 10.00, 20, TRUE)
                 RETURNING *`,
            ...
            const prodBRes = await db.query(
                `INSERT INTO products (artisan_id, category_id, name, price, stock_quantity, is_active)
                 VALUES ($1, $2, 'Prod B Ord Zero Stock', 15.00, 0, TRUE)
                 RETURNING *`,
                [artigiano1Id, categoryId]
            );
            ...
            const prodCRes = await db.query(
                `INSERT INTO products (artisan_id, category_id, name, price, stock_quantity, is_active)
                 VALUES ($1, $2, 'Prod C Ord Art2', 20.00, 30, TRUE)
                 RETURNING *`,
    ...
    describe('PUT /api/orders/:id/status', () => {
        ...
        it('Admin: dovrebbe aggiornare stato SHIPPED -> DELIVERED', async () => {
            await request(app).put(`/api/orders/${orderToModifyId}/status`).set('Authorization', `Bearer ${adminToken}`).send({status: 'paid'}).expect(200);
            await request(app).put(`/api/orders/${orderToModifyId}/status`).set('Authorization', `Bearer ${adminToken}`).send({status: 'processing'}).expect(200);
            await request(app).put(`/api/orders/${orderToModifyId}/status`).set('Authorization', `Bearer ${adminToken}`).send({
                status: 'shipped',
                tracking_number: 'T'
            }).expect(200);
